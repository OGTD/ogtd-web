...
(defn fupdate [value-or-function]
...
(defn inject-state
  ([component & keys]
   (r/create-class
    {:context-types #js {:__freactalish-context js/PropTypes.object}
     :reagent-render (fn [props & children]
                       (let [this (r/current-component)
                             context (gobj/get (.-context this) "__freactalish-context")
                             state (:state context)
...
(defn provide-state
  ([component store]
   (let [state (reduce-kv (fn [m k v] (assoc m k (r/atom v))) {} (:state store))
     ...
     (r/create-class
      {:child-context-types #js {:__freactalish-context js/PropTypes.object}
       :context-types #js {:__freactalish-context js/PropTypes.object}
       :get-child-context #(this-as this
                             (let [parent-context (or (gobj/get (.-context this) "__freactalish-context") #js {})
                                   merged-state (merge (or (:state parent-context) {}) state)
                                   merged-effects (merge (or (:effects parent-context) {}) @effectsv)
                                   context #js {:__freactalish-context {:state merged-state :effects merged-effects}}]
                               context))
       :reagent-render (fn [props & children]
